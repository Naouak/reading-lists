...
export default {
  ...
  asyncData({$axios}) {
    const recentPromise = $axios.$get('/book/?limit=50&exclude_term=Infinity Comic,Trade Paperback&ordering=-availability_date&only_available=1&only_published=1').then(r => r.results);
    const newPromise = $axios.$get('/book/?limit=50&exclude_term=Infinity Comic,Trade Paperback&ordering=-pub_date,title&only_available=1&only_published=1').then(r => r.results);
